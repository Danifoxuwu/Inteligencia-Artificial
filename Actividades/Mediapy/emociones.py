...
# Inicializar MediaPipe Face Mesh y Haar cascades
...
# Unir todas las conexiones en una sola estructura
FACEMESH_CONTOURS = frozenset().union(FACEMESH_LIPS, FACEMESH_LEFT_EYE, FACEMESH_RIGHT_EYE, FACEMESH_FACE_OVAL)
...
def detectar_emocion(medidas):
    """Detecta emociones basadas en las medidas faciales."""
    if medidas["distancia_boca"] > medidas["distancia_ojos"] * 0.8:
        return "Feliz"  # Boca abierta
    elif medidas["distancia_ojos"] < 5:  # Umbral para ojos cerrados
        return "Triste"  # Ojos cerrados
    else:
        return "Serio"  # Ninguna de las anteriores

def detectar_vida(frame, prev_frame):
    # Detectar movimiento basado en diferencias entre fotogramas
    diff = cv2.absdiff(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY), cv2.cvtColor(prev_frame, cv2.COLOR_BGR2GRAY))
    movimiento = np.sum(diff > 25) / diff.size  # Porcentaje de píxeles con cambio significativo

    # Detectar cambios de luz
    luz_actual = np.mean(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY))
    luz_anterior = np.mean(cv2.cvtColor(prev_frame, cv2.COLOR_BGR2GRAY))
    cambio_luz = abs(luz_actual - luz_anterior)

    # Verificar si hay movimiento y cambios de luz
...
def draw_landmarks_and_connections(frame, puntos, connections):
    ...
    for start, end in connections:
        if start in puntos and end in puntos:
            cv2.line(frame, puntos[start], puntos[end], (255, 0, 0), 1)  # Líneas en azul
...
while cap.isOpened():
    ...
    # Detectar si el sujeto está vivo
    if prev_frame is not None:
        vivo = detectar_vida(frame, prev_frame)
        cv2.putText(frame, f"Vivo: {'Si' if vivo else 'No'}", (10, 60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0) if vivo else (0, 0, 255), 2)
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            # Calcular medidas relevantes usando FACEMESH_CONTOURS
            if all(k in puntos for k in [33, 133, 362, 263, 61, 291]):
                medidas_actuales = {
                    "distancia_ojos": distancia(puntos[33], puntos[133]),  # Ojo izquierdo a derecho
                    "distancia_boca": distancia(puntos[61], puntos[291]),  # Labio superior a inferior
                }
